...
class GeneticAlgorithm2(GeneticAlgorithm):
    ...
    def initialize(self):
        self.population1, self.population2 = self._generate_random_valid_solutions()
        self.population = np.concatenate((self.population1, self.population2))
    ...
    def search(self, n_iterations, report=False, log=False):
        ...
        for iteration in range(n_iterations):
            ...
            if self.repetition1>2 and self.flag1:
                ...
                self.selection1 = uls.parametrized_tournament_selection(self.radius)
            ...
            if self.repetition1>2 and self.flag1==False:
                ...
                self.p_c1 = 0
                self.p_m1 = 0.9
                self.mutation1 = uls.parametrized_ball_mutation(self.radius)
            ...
            if self.repetition2>2 and self.flag2:
                print(">>>>>>>>>>>>>>>>>>>>>>>>> TRUE <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
                self.p_c2=0
                self.selection = uls.parametrized_tournament_selection(self.radius)
                #self.selection2 = uls.parametrize_roulette_wheel()
                self.p_m2 = .9
                self.repetition2 = 0
                self.flag2 = False
                self.mutation2 = uls.parametrized_ball_mutation(self.radius)
            ...
            while len(offsprings1) < len(self.population1):
                off11, off12 = p11, p12 = [
                    self.selection1(self.population1, self.problem_instance.minimization, self._random_state) for _ in
                    range(2)]
            ...
            newPopulation1 = selectTheBestOffspringsList1[:18] + selectTheBestPopulationList1[:7]
            ...
            print("MIN-Population1", uls.calculate_min_solution(newPopulation1).fitness)
            print("\n")
           ...
           # print("MAX-Offsprings1: ", uls.calculate_max_solution(offsprings1).fitness)
            ...
            newPopulation2 = selectTheBestOffspringsList2[:24] + selectTheBestPopulationList2[:1]
            ...
            print("MIN-Population2", uls.calculate_min_solution(newPopulation2).fitness)
    ...
    def _crossover1(self, p1, p2):
        off11, off21 = self.crossover2(p1.representation, p2.representation, self._random_state)
        off11, off21 = Solution(off11), Solution(off21)
        return off11, off21
    ...
    def _mutation1(self, individual):
        mutant1 = self.mutation1(individual.representation, self._random_state, self.radius)
        mutant1 = Solution(mutant1)
        return mutant1

    def _mutation2(self, individual):
        mutant2 = self.mutation2(individual.representation, self._random_state,  self.radius)
